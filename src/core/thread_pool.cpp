...
ThreadPool::ThreadPool(uint32_t threads_num) : m_nr_threads(threads_num), m_stop{false}, m_active{false} {
    if (threads_num < 1)
    {
        m_nr_threads = 1;
    }
    if (m_nr_threads > 1)
    {
        ...
        {
            INFER_LOG("The number of threads is bigger than number of physical cpu cores, got: %d core_number: %d", system_cpu_count, nr_threads());
        }
...
inline void ThreadPool::sync() {
    ...
    do {
        ...
        if (no_finished) {
            ...
            {
                if (!m_workers[no_finished_id]->work_flag.load(std::memory_order_acquire)) {
                    break;
                }
                if ((it < ACTIVE_WAIT_PAUSE_LIMIT || (it & 1))) {
                    INFER_PAUSE(16);
